...
# Попытка импорта matplotlib для генерации красивых графиков
try:
    import matplotlib
    matplotlib.use('Agg')  # Безэкранный режим для серверов (Linux/Docker)
...
# ================================ КОНФИГУРАЦИЯ =================================
# Бот сначала ищет переменные в системе (для серверов Render/Docker), 
# а если не находит — берет то, что вы вставите в кавычки ниже.
TELEGRAM_TOKEN = os.getenv("TELEGRAM_TOKEN", "ВСТАВЬ_СВОЙ_ТОКЕН_СЮДА")
GROQ_API_KEY = os.getenv("GROQ_API_KEY", "ВСТАВЬ_СВОЙ_КЛЮЧ_GROQ_СЮДА")
...
PP_TIPS = [
    ...
    "Сладкий перец содержит больше витамина C, чем цитрусовые.",
...
def calculate_macros(weight, height, age, gender, activity, goal):
    ...
    # Расчет по 3 вашим целям
    if goal == "Похудение (Сушка)":
        kcal = tdee * 0.82  # Безопасный дефицит 18%
        p = weight * 2.0    # Повышенный белок для сохранения мышц
        f = weight * 0.8    # Умеренные жиры
    elif goal == "Набор массы":
        kcal = tdee * 1.15  # Профицит 15%
        p = weight * 1.7
        f = weight * 1.1
    else:  # Поддержание
        kcal = tdee
...
def calculate_water_target(weight):
    # Научная норма: 35 мл чистой воды на 1 кг массы тела
...
async def check_subscription(message_or_call) -> bool:
    ...
    # Если канал не указан или остался дефолтным — пропускаем проверку
    if not CHANNEL_ID or CHANNEL_ID == "@твой_канал" or CHANNEL_ID == "ВСТАВЬ_СВОЙ_ТОКЕН_СЮДА":
        return True
    ...
    try:
        ...
        return True  # В случае сбоя API пропускаем пользователя

    # Если не подписан, выводим красивую карточку
    ...
    text = (
        ...
        "Это помогает нам развивать и поддерживать бота абсолютно бесплатным для вас! 😊"
    )
...
def get_main_keyboard():
    ...
    kb.add("📝 Внести еду", "📊 Моё БЖУ", "👤 Мой профиль", "📔 Дневник питания", 
           "💡 Совет дня", "💧 Вода +250мл", "📈 Прогресс", "🤖 AI Ассистент")
    return kb
...
async def show_profile(message: types.Message):
    ...
    if not p:
        await ProfileStates.weight.set()
        await message.answer("Сначала заполни анкету. Напиши свой <b>вес (в кг)</b>:")
        return
    ...
    text = (
        ...
        f"📏 <b>Рост:</b> {p[2]} см\n"
        f"⚖️ <b>Вес:</b> {p[1]} кг\n"
        f"🎂 <b>Возраст:</b> {p[3]} лет\n"
        f"🎯 <b>Цель:</b> {p[6]}\n"
        f"💧 <b>Норма воды:</b> {calculate_water_target(p[1])} мл\n\n"
        f"🔥 <b>Целевые калории:</b> {p[7]} ккал"
    )
    
    kb = types.InlineKeyboardMarkup()
    kb.add(types.InlineKeyboardButton("⚖️ Быстро обновить вес", callback_data="update_weight_start"))
    kb.add(types.InlineKeyboardButton("⚙️ Изменить все параметры", callback_data="change_profile_data"))
    
    await message.answer(text, reply_markup=kb)

# --- ⚖️ БЫСТРОЕ ОБНОВЛЕНИЕ ВЕСА ---
...
async def update_weight_start(call: types.CallbackQuery):
    ...
    await call.message.answer("Введи свой новый вес (в кг):")
    await call.answer()
...
async def process_new_weight(message: types.Message, state: FSMContext):
    try:
        ...
        # Пересчитываем КБЖУ с новым весом, сохраняя цель и активность
        new_kcal, new_p, new_f, new_c = calculate_macros(new_w, p[2], p[3], p[4], p[5], p[6])
        save_user_profile(user_id, new_w, p[2], p[3], p[4], p[5], p[6], new_kcal, new_p, new_f, new_c)
        
        await state.finish()
        await message.answer(
            f"✅ <b>Вес успешно обновлен!</b>\n\n"
            f"Новые нормы пересчитаны автоматически:\n"
            f"🔥 <b>Калории:</b> {new_kcal} ккал\n"
            f"💧 <b>Норма воды:</b> {calculate_water_target(new_w)} мл", 
            reply_markup=get_main_keyboard()
        )
    except ValueError:
        await message.answer("Ошибка! Введи вес числом (например, 70.5):")

# --- ⚙️ ПОЛНОЕ ИЗМЕНЕНИЕ ДАННЫХ (БЕЗ смены цели) ---
...
async def my_macros(message: types.Message):
    ...
    kb.add(
        ...
        types.InlineKeyboardButton("👤 Открыть Профиль", callback_data="change_profile_redirect")
    )
    
    await message.answer(text, reply_markup=kb)

@dp.callback_query_handler(lambda c: c.data == "change_profile_redirect", state='*')
async def cb_profile_redirect(call: types.CallbackQuery):
    p = get_user_profile(call.from_user.id)
    if p:
        text = (
            f"👤 <b>ТВОЙ ПРОФИЛЬ:</b>\n\n"
...
async def show_progress(message: types.Message):
    ...
    # Попытка отрисовать красивую диаграмму через matplotlib
    if HAS_MATPLOTLIB:
        ...
        # Если библиотеки нет на сервере — выводим текстовую версию
...
async def cb_delete_meal(call: types.CallbackQuery):
    ...
    await call.answer("Блюдо удалено из дневника!")
...
# --- 🤖 AI АССИСТЕНТ (С ПАМЯТЬЮ И ДАННЫМИ) ---
...
# --- ЭХО ОБРАБОТЧИК НЕИЗВЕСТНЫХ КОМАНД ---
        ...
        if now.hour == 10 and now.minute == 0:
            ...
            for user in users:
                ...
                try:
                    ...
                    await asyncio.sleep(0.05)  # Защита от лимитов отправки
                except Exception as e:
