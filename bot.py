...
# Используем aiohttp, который гарантированно установлен
import aiohttp
...
# ================================ КОНФИГУРАЦИЯ =================================
# Бот сначала попытается взять ключи из настроек твоего хостинга (переменных окружения).
# Если их там нет, он возьмет значения, которые ты впишешь ниже вручную.
...
CHANNEL_ID = os.getenv("CHANNEL_ID", "@твой_канал") # ID твоего канала с @
...
# Проверка на дурака, чтобы бот не падал с непонятной ошибкой, если забыл поменять токен
if "СЮДА_ВСТАВЬ" in TELEGRAM_TOKEN or TELEGRAM_TOKEN == "":
    logging.error("КРИТИЧЕСКАЯ ОШИБКА: Ты забыл заменить 'СЮДА_ВСТАВЬ_ТОКЕН_ОТ_BOTFATHER' на твой настоящий токен от @BotFather!")
    exit(1)

bot = Bot(token=TELEGRAM_TOKEN, parse_mode="HTML")
storage = MemoryStorage() # Хранилище состояний в оперативной памяти (для aiogram 2.x)
...
def get_main_keyboard():
    ...
    keyboard.add(types.KeyboardButton(text="🏠 Menu"), types.KeyboardButton(text="⬅️ Назад"))
...
async def ask_groq_ai(payload: dict) -> str:
    """Отправляет запрос напрямую в API Groq без использования сторонних библиотек SDK."""
    headers = {
...
async def cmd_start(message: types.Message, state: FSMContext):
    ...
    if profile:
        ...
        await message.answer(
            f"💪 С возвращением, <b>{message.from_user.first_name}</b>! Твои параметры:\n"
            f"⚖️ <b>Вес:</b> {weight} кг\n"
            f"📏 <b>Рост:</b> {height} см\n"
            f"🚻 <b>Пол:</b> {gender}\n\n"
            f"Я готов к работе! Пришли мне фото еды или используй кнопки ниже. "
...
# --- 2. Обработчик кнопки "🏠 Menu" / "🏠 Меню" ---
@dp.message_handler(lambda message: message.text in ["🏠 Menu", "🏠 Меню"], state='*')
...
async def process_gender(message: types.Message, state: FSMContext):
    if message.text in ["Мужской 👦", "Женский 👧"]:
        ...
        await message.answer(
            ...
            "Теперь ты можешь пользоваться ботом без каких-либо ограничений.",
            reply_markup=get_main_keyboard()
        )
    else:
...
# --- Логика обработки текстового запроса ---
    ...
    payload = {
        ...
        "messages": [
            {"role": "system", "content": 
             ...
             "В конце обязательно добавь примечание, что расчет приблизительный."},
...
async def process_food_photo(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(
            "⚠️ Не удалось проанализировать фото. Убедись, что на фото действительно еда и API ключ Groq настроен верно.",
            reply_markup=get_main_keyboard()
        )
