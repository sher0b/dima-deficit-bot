...
# Названия моделей (если одна не работает, можно легко заменить здесь)
...
async def p_g(message: types.Message, state: FSMContext):
    ...
    text = (
        "Пожалуйста, выберите коэффициент активности:\n\n"
        "• 1.2 – сидячий (офис, без спорта)\n"
        "• 1.3 – небольшая (5-8к шагов, легкий спорт)\n"
        "• 1.4 – умеренная (8-12к шагов, 2-3 тренировки)\n"
        "• 1.5 – высокая (12к+ шагов, 3-5 тренировок)\n"
        "• 1.6 – очень высокая (физ. труд + ежедневный спорт)"
    )
