...
# Используем aiohttp, который уже есть в системе благодаря aiogram
import aiohttp
...
def get_main_keyboard():
    ...
    keyboard.add(types.KeyboardButton(text="🏠 Menu"), types.KeyboardButton(text="⬅️ Назад")) # Исправлено на Menu / Меню
...
async def process_food_text(message: types.Message, state: FSMContext):
    ...
    # Формируем JSON-тело для Groq API
    payload = {
...
async def process_food_photo(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        # Формируем JSON payload для Vision-запроса к Groq
        payload = {
            "model": "llama-3.2-11b-vision-preview",
            "messages": [
                {
