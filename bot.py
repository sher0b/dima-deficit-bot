...
# Настройки вашего Telegram-канала для обязательной подписки
...
# ============================ РАСЧЕТ КБЖУ ПО ОБНОВЛЕННЫМ ЦЕЛЯМ ============================
...
# ============================ ЗАПРОС К ИИ (С ПАМЯТЬЮ И ДАННЫМИ ПРОФИЛЯ) ============================
...
def get_main_keyboard():
    ...
    kb.add("📝 Внести еду", "📊 Моё БЖУ", "🤖 AI Ассистент", "📔 Дневник питания", "💡 Совет дня")
    return kb
...
# --- Изменение цели из инлайна (без ввода всех данных) ---
...
async def cb_set_goal(call: types.CallbackQuery):
    ...
    await call.message.answer(
        ...
        f"Новые параметры КБЖУ пересчитаны:\n"
...
# --- Изменение профиля (перезапуск анкеты) ---
...
async def cb_change_profile(call: types.CallbackQuery, state: FSMContext):
    await state.finish()
    await ProfileStates.weight.set()
    await call.message.answer("Окей, обновим твои параметры.\nНапиши актуальный <b>вес (в кг)</b>:")
    await call.message.delete()
    await call.answer()
...
# --- Режим AI Ассистента (с памятью и контекстом профиля) ---
...
async def ai_chat(message: types.Message, state: FSMContext):
    ...
    # 1. Загрузка параметров пользователя для контекста
    p = get_user_profile(message.from_user.id)
    if p:
    ...
    # 3. Загрузка истории диалога из БД (память на последние 6 сообщений)
    ...
    try:
        ...
        # Сохраняем переписку в историю БД
...
# ============================ ЕЖЕДНЕВНАЯ РАССЫЛКА (1 РАЗ В ДЕНЬ В 10:00) ============================
async def daily_scheduler():
    while True:
        now = datetime.now()
        # Каждое утро в 10:00 рассылаем напоминание
            ...
            for user in users:
                ...
                try:
                    await bot.send_message(
                        ...
                        "Стабильность — твой главный союзник на пути к красивой и здоровой форме! 💪",
                        reply_markup=get_main_keyboard()
                    )
                    await asyncio.sleep(0.05)  # Ограничение против лимитов Telegram (anti-flood)
                except Exception as e:
                    logging.error(f"Не удалось отправить рассылку пользователю {user_id}: {e}")
            await asyncio.sleep(65)  # Пропускаем эту минуту во избежание дублирования
        await asyncio.sleep(30)  # Спим полминуты перед новой проверкой

# ================================= ЗАПУСК =================================
if __name__ == "__main__":
    # Запускаем ежедневную рассылку в фоновом режиме внутри общего Event Loop
